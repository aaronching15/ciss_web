...
try:
    ...
    if not isinstance(data, c.EmQuantData):
        ...
        if(data.ErrorCode != 0):
            ...
            print("Output data:",type(data) , data )
            for code in data.Codes:
                for i in range(0, len(data.Indicators)):
    ...
    # 收盘价等指标 8列
    ### columns=  INDEXCODE   SECUCODE   TRADEDATE  NAME  CLOSE  ... CONTRIBUTEPT SHRMARKETVALUE       MV TOTALTRADABLE SHARETOTAL
    ...
    ###########################################################################
    # cfc使用范例;给定代码和指标获取指标值  ？？啥指标
    ### data.Indicators = INDEX  CODE  INDICATOR 
    ### type(data) = class 'EmQuantAPI.c.EmQuantData';包含 ErrorCode=0, ErrorMsg=success, Data={。。。}
    ### 字典类型，data.Data= Data={'0': [1, '000001.SZ', True, '000001.SZ'], '1': [2, '000001.SH', True, '000001.SH'], '2': [3, '600000.SH', True, '600000.SH'], '3': [4, '000000.TEST', False, None]}
