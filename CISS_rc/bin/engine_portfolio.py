...
===============================================
VIP bug: line 529 
    现在是初期根据所有交易日交易计划去更新组合，面对逐日更新的情况，还需要大幅更改！


Function: realize engine(loop) function for portfolio simulation 
功能：实现模拟组合的循环测试和验证
last update 181108 | since 181108
...
notes:
    1,engine 主要是定义计算过程和如何计算，具体计算执行和数据操作放在db\\ports.py 里
    2,general operation should be set in ports.py\\manage_portfolios.py 
...
class Engine_ports():
    ...
    def test_abm_1port_Nperiods_ind_x(self,ind_level,sty_v_g,date_periods,int_ind_x,port_name,init_cash,dataG,df_tb_fi_fi, df_tb_fi_cap,abm_model) :
        '''
        ...
        for temp_i in range( len(date_periods.periods_start) ) :
            ...
            if if_period_1st == 0 :
                ...
                (portfolio_manage,portfolio_suites) = self.gen_abm_1port_1period_ind_x(ind_level,sty_v_g,port_name,int_ind_x,date_start,date_end,date_reference_change,init_cash,dataG,df_tb_fi_fi, df_tb_fi_cap,abm_model) 
            else :
                # port_name = portfolio_manage.portfolio_head["portfolio_name"] || port_id = portfolio_manage.portfolio_head["portfolio_id"] 
                (portfolio_manage,portfolio_suites) = self.update_abm_1port_1period_ind_x(ind_level,sty_v_g,portfolio_manage,int_ind_x,date_start,date_end,date_reference_change,init_cash,dataG,df_tb_fi_fi, df_tb_fi_cap,abm_model) 

        return portfolio_manage,portfolio_suites


    def gen_abm_1port_1period_ind_x(self,ind_level,sty_v_g,port_name,int_ind_x,date_start,date_end,date_reference_change,init_cash,dataG,df_tb_fi_fi, df_tb_fi_cap,abm_model) :
        '''
        ...
        ### step1 Get symbol list for all indexes 
        ...
        ### step2 Calculate financial estimates for current period
        ...
        ### step3 calculate for anchor stocks in value and growth perspective
        # Notes: if ind_level='0', we use level 1 here 
        if ind_level == '0' : 
            ind_level2 ='1'
        else :
            ind_level2 = ind_level
        [temp_list,cols_new_anchor] = abm_model.calc_anchor_stocks(temp_list,ind_level2)
        # it is fine until here, only 3 stock with no profit estimates
        # temp_list.to_csv("D:\\temp_list_anchor_181121.csv")

        ### step4 calculate shadow enterprise value from anchor stocks in value and growth perspectives
        [temp_list,cols_new_shadow] = abm_model.calc_shadow_ev_from_anchor(temp_list, ind_level2 ) 
        # temp_list.to_csv("D:\\temp_list_anchor_2_181121.csv")

        ### step4 Save result to csv files 
        path0 = "D:\\CISS_db\\abm_weights\\"
        file_name = "weights_"+date_reference_change.replace("-","") +"_"+str(ind_level) +".csv"
        file_name2 = "weights_"+date_reference_change.replace("-","") +"_"+str(ind_level2) +".csv"
        import os
        ...
        # if ind_level == "0" :
        if ind_level == "0" :
            temp_list.to_csv(path0 + file_name2  )
            temp_list.to_csv(path0 + file_name  )
        else :
            temp_list.to_csv(path0 + file_name  )
        ...
        ### Available for ind_level == '0' since 190412
        # if not ind_level == '0' :
        ...
        print(temp_df_value["w_allo_value_ind"+ind_level2].sum() )
        print(temp_df_value["w_allo_value_ind"+ind_level2] ) 
        ...
        print(temp_df_growth["w_allo_growth_ind"+ind_level2].sum() )
        print(temp_df_growth["w_allo_growth_ind"+ind_level2] ) 
        ...
        #     abm_model = Abm_model()
        #     temp_df_growth = abm_model.output_anchor_list_indX(ind_level,date_reference_change   )
        #     # sum of w_allo_value_ind1 is 11 here 
        #     ##############################################################################
        #     ### Method 1 |11个行业以行业角度平均权重
        #     # temp_df_growth["w_allo_value_ind1"] = temp_df_growth["w_allo_value_ind1"]/temp_df_growth["w_allo_value_ind1"].sum()
        #     # temp_df_growth["w_allo_growth_ind1"] = temp_df_growth["w_allo_growth_ind1"]/temp_df_growth["w_allo_growth_ind1"].sum()
            
        #     ##############################################################################
        #     ### Method 2 |11个行业以全市场预测净利润角度平均权重 "profit_q4_es"
        #     df_profit_posi = temp_df_growth[ temp_df_growth["profit_q4_es"]>0 ]
        #     df_profit_dif_posi = temp_df_growth[ temp_df_growth["profit_q4_es_dif"]>0 ]
        #     temp_df_growth["w_allo_value_ind1"] = temp_df_growth["profit_q4_es"]/df_profit_posi["profit_q4_es"].sum()
        #     temp_df_growth["w_allo_growth_ind1"] = temp_df_growth["profit_q4_es_dif"]/df_profit_dif_posi["profit_q4_es_dif"].sum()
            
        #     ### get rid of negative values and make sum of w_allo_ to 1 
        #     temp_df_growth["w_allo_value_ind1"] =temp_df_growth["w_allo_value_ind1"].apply(lambda x :max(x,0) )
        #     temp_df_growth["w_allo_growth_ind1"] =temp_df_growth["w_allo_growth_ind1"].apply(lambda x :max(x,0) )
            

        #     temp_df_value =temp_df_growth 
    ...
    def update_abm_1port_1period_ind_x(self,ind_level,sty_v_g,portfolio_manage,int_ind_x,date_start,date_end,date_reference_change,init_cash,dataG,df_tb_fi_fi, df_tb_fi_cap,abm_model) :
        '''
        ind_level="1"
        ...
        ### step4 Save result to csv files 
        path0 = "D:\\CISS_db\\abm_weights\\"
        file_name2 = "weights_"+date_reference_change.replace("-","") +"_"+str(ind_level) +".csv"
        file_name = "weights_"+date_reference_change.replace("-","") +"_"+str(ind_level2) +".csv"
        import os
        if not os.path.isdir( path0 ) :
            os.makedirs(path0)  
        # temp_df_growth.to_csv(temp_path + "temp_df.csv"  )
        ...
        temp_list = temp_list.drop(["Unnamed: 0"],axis=1)
        # int to str
        col_name_ind = 'ind'+ ind_level2 + '_code' # 'ind1_code'
        temp_list[col_name_ind]=temp_list[col_name_ind].astype(str)
        ##################################################################
        print("temp_list has been loaded.")
        # print( temp_list.info())
        ...
        if not int_ind_x == "999" :
            ...
            ##############################################################################
            ### Method 1 |11个行业以行业角度平均权重
            # temp_df_value["w_allo_value_ind1"] = temp_df_value["w_allo_value_ind1"]/temp_df_value["w_allo_value_ind1"].sum()
            # temp_df_value["w_allo_growth_ind1"] = temp_df_value["w_allo_growth_ind1"]/temp_df_value["w_allo_growth_ind1"].sum()
            
            ##############################################################################
            ### Method 2 |11个行业以全市场预测净利润角度平均权重 "profit_q4_es"
            df_profit_posi = temp_df_value[temp_df_value["profit_q4_es"]>0 ]
            df_profit_dif_posi =temp_df_value[ temp_df_value["profit_q4_es_dif"]>0 ]
            temp_df_value["w_allo_value_ind1"] = temp_df_value["profit_q4_es"]/df_profit_posi["profit_q4_es"].sum()
            temp_df_value["w_allo_growth_ind1"] = temp_df_value["profit_q4_es_dif"]/df_profit_dif_posi["profit_q4_es_dif"].sum()
            ### get rid of negative values and make sum of w_allo_ to 1 
            temp_df_value["w_allo_value_ind1"]  =temp_df_value["w_allo_value_ind1"].apply(lambda x :max(x,0) )
            temp_df_value["w_allo_growth_ind1"] =temp_df_value["w_allo_growth_ind1"].apply(lambda x :max(x,0) )
        ...
        ### Growth allocation in industry level 3 
        if not int_ind_x == "999" :
            temp_df_growth = temp_list[ temp_list[col_name_ind ] == int_ind_x  ]
        else :
            temp_df_growth = temp_list
            ##############################################################################
            ### Method 1 |11个行业以行业角度平均权重
            # temp_df_growth["w_allo_value_ind1"] = temp_df_growth["w_allo_value_ind1"]/temp_df_growth["w_allo_value_ind1"].sum()
            # temp_df_growth["w_allo_growth_ind1"] = temp_df_growth["w_allo_growth_ind1"]/temp_df_growth["w_allo_growth_ind1"].sum()
        ...
        # Straetgy could be a roughly estimation of how many stock to trade
        # 策略是粗线条的，只说我们要买入多少比例的股票
        # 若signals交易信号中资产配置比例数值有问题，需要进行调整 | 190413
        stra_weight_list = stra_allocation('').stock_weights(ind_level,sty_v_g, stockpool_df)
        print('weight_list of strategy:')
        print(stra_weight_list)
        ...
        portfolio_suites = manager_trades.manage_tradeplan(if_rebalance,ind_level2,sty_v_g,portfolio_suites, signals_list, config_IO_0 ,date_start,date_end,quote_type,data_wind_0 )
