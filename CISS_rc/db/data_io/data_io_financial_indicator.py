...
1，
last | since 20201022
功能：导入各类季度、年度财务相关的数据、指标等
1,历史财务数据类
2，卖方财务预测相关类数据



derived from def import_data_financial_ana;data_io_factor_model.py
===============================================
'''
import sys,os
import pandas as pd
...
#######################################################################
class data_io_financial_indicator():
    def __init__(self):
        #######################################################################
    ...
    def print_info(self):        
        print("   ")
        ### 设置参数Config：根据输入的参数，确定需要提取数据的区间和相关表格
        print("config_data_financial_estimate |根据输入的参数，确定需要提取财务和预测数据的区间和相关表格 ")
        
        ### 数据导入I/O：财务数据和财务指标
        print("import_data_financial |导入财务数据和财务指标 ")
        ###导入卖方预期数据
        print("import_data_estimate |导入卖方预期数据 ")

        ### 数据导出I/O：财务数据和财务指标
        print("export_data_financial |输出财务数据和财务指标 ")
 
        

        return 1

    def config_data_financial_estimate(self, obj_fi ):
        ### 根据输入的参数，确定需要提取财务和预测数据的区间和相关表格
        '''
        obj_fi["df_ashare"] = df_abcd3d,包括了所有a股的代码、行业分类等数据
        wds数据表里和财务分析有关的指标:A股财务类 config_indi_financial_1.obj_config["df_col_fi_indi_CN_factor"]
        上一步对象： config_indi_financial_1
        '''
        ########################################################################
        ### 导入参数：
        temp_date = obj_fi["dict"]["date"] #  "20201022"
        
        ########################################################################
        ### 获取最近的N个季度财务披露日期
        obj_date = {}
        obj_date["date"] = temp_date 
        ### 默认取最近8个季度和5个年度的日期
        obj_date["N_quarter"] = 8
        obj_date["N_year"] = 5  
        
        obj_date = self.data_io_1.get_report_date(obj_date)     
        obj_fi["dict"]["para_date_N"] = obj_date["para_date_N"] 
        obj_fi["dict"]["date_list_q_N"] = obj_date["date_list_q_N"] 
        obj_fi["dict"]["date_list_y_N"] = obj_date["date_list_y_N"] 

        ########################################################################
        ### 获取对应的wds财务表格tables和相应的columns
        ### 中国A股财务指标AShareFinancialIndicator里的指标column分类：
        '''
        将列信息分为5类：收入和利润,现金流,费用管理,资产和投资收益,负债和利息
                        table  if_needed type_financial factor  weight_indicator          name_CN                    name  notes
        0  AShareFinancialIndicator          1        资产和投资收益     价值               NaN           营运流动资本  S_FA_NETWORKINGCAPITAL    NaN
        1  AShareFinancialIndicator          1        资产和投资收益     价值               NaN           全部投入资本      S_FA_INVESTCAPITAL    Na
        '''
        obj_fi["dict"]["df_col_fi_indi_CN_factor"] = config_indi_financial_1.obj_config["df_col_fi_indi_CN_factor"]
        obj_fi["dict"]["df_col_esti_indi_CN_factor"] = config_indi_financial_1.obj_config["df_col_esti_indi_CN_factor"]
  
        return obj_fi

    def import_data_financial(self, obj_fi ):
        ### 导入财务数据和财务指标
        '''
        obj_fi["df_ashare"] = df_abcd3d,包括了所有a股的代码、行业分类等数据
        上一步：   config_data_financial(self, obj_fi )     
        notes:
        1，不同日期股票池会发生变化，因此要注意股票上市和退市的情况。
        2，由于不断有新股上市，而早期财务报告里没有新股，因此未来还是需要opdate模式更新数据
        '''
        ########################################################################
        ### 导入参数：季度和年度末都是日期从新往旧排列
        ...
        ########################################################################
        ### 获取最近的N个季度财务披露日期，date_list_q_N，obj_date["N_quarter"] = 8
        # 目前只有1个table：AShareFinancialIndicator
        table_list = list( df_col_fi_indi_CN_factor["table"].drop_duplicates() )
        
        path_temp = config_data_1.obj_config["dict"]["path_wind_wds"]
        ### 取不同wds表格
        for table_name in table_list :
            obj_fi[ table_name ] ={}
            df_sub = df_col_fi_indi_CN_factor[df_col_fi_indi_CN_factor["table"]==table_name ]

            ### keep only needed columns
            df_sub1 = df_sub[ df_sub["if_needed"]==1 ]
            col_list = list( df_sub1["name"].values )
            ### 定位信息如代码、日期等
            # col_list_basic=["S_INFO_WINDCODE","WIND_CODE","ANN_DT","REPORT_PERIOD" ]
            df_sub2 = df_sub[ df_sub["if_needed"]==2 ]
            col_list_basic = list( df_sub2["name"].values )

            ### 期末数据文件前缀,后缀
            file_prefix = df_sub["file_prefix"].values[0]
            file_sufix  = df_sub["file_sufix"].values[0]

            path_table = path_temp + table_name +"\\"
            ### 对每个季度提取数据：
            for temp_date in date_list_q_N:
                ### WDS_REPORT_PERIOD_20191231_ALL                
                file_name = file_prefix + temp_date + file_sufix + ".csv"
                df0 = pd.read_csv(path_table + file_name  )

                ### 仅保留部分columns
                obj_fi[ table_name ][temp_date] = df0.loc[:, col_list_basic+col_list ]
                
        ########################################################################
        ### 获取最近的N个年度财务披露日期，date_list_y_N，obj_date["N_year"] = 5 
        # # 目前只有1个table：AShareFinancialIndicator 
        # notes:季末日期和年末日期有可能重合，这是避免重复导入
        table_list = list( df_col_fi_indi_CN_factor["table"].drop_duplicates())
        obj_fi["dict"]["table_list"] = table_list
        ...
        for table_name in table_list :
            ...
            path_table = path_temp + table_name +"\\"
            ### 对每个年度提取数据：
            for temp_date in date_list_y_N:
                if temp_date not in obj_fi[ table_name ].keys() :
                    ### WDS_REPORT_PERIOD_20191231_ALL                
                    file_name = file_prefix + temp_date + file_sufix + ".csv"
                    try :
                        df0 = pd.read_csv(path_table + file_name  )
                    except:
                        df0 = pd.read_csv(path_table + file_name,encoding="gbk"  )
    ...
    def import_data_estimate(self,obj_fi):
        ### 导入卖方预期数据
        '''
        按交易日发布的盈利预测明细数据
        wds数据表类似：预测数据：
        1，机构对个股的盈利预测汇总计算的综合值，30，90，180天：中国A股盈利预测汇总[AShareConsensusData]，
        2，每日滚动生成A股个股一致预测指标，Wind一致预测个股滚动指标[AShareConsensusRollingData]
        3，逐个研究员对A股的盈利预测数据：中国A股盈利预测明细[AShareEarningEst]
        4，包括宏观和策略所有研报类型的行业投资评级信息：中国A股行业投资评级[AShareIndusRating]
        
        ana:
        1，时间选择：前推的卖方预测数据通常跟随每个季度报告调整，卖方覆盖度低的股票可能半年甚至2年都没有卖方覆盖。
            选取近10、30、90、180天足够了，对应交易日差不多10、25、120
        2，近5天如果研报数量密集增加，则需要小心见顶迹象：
        例子：智飞生物在200430、200622、200804都出现了密集的卖方研报推荐，之后1个月股价都出现了回调。

        '''
        ########################################################################
        ### 导入参数：
        df_col_esti_indi_CN_factor = obj_fi["dict"]["df_col_esti_indi_CN_factor"] 

        # 需要获取区间内的交易日列表，按交易日获取数据；交易日数据不需要多次计算 
        temp_date = obj_fi["dict"]["date"] 

        ### 需要获取区间内的交易日列表，按交易日获取数据
        date_list_tradingday = self.obj_data_io["dict"]["date_list_tradingday"]  
        ### 获取过去30~180个交易日的数据
        tradingday_lenth = 30
        date_list_sub = [ x for x in date_list_tradingday if x<= int(temp_date) ][-1*tradingday_lenth :]

        
        ########################################################################
        # 目前有2个table：汇总值AShareConsensusData 和明细值AShareEarningEst
        table_list = list( df_col_esti_indi_CN_factor["table"].drop_duplicates())
        path_temp = config_data_1.obj_config["dict"]["path_wind_wds"]
        ...
        for table_name in table_list : 
            ...
            df_sub = df_col_esti_indi_CN_factor[df_col_esti_indi_CN_factor["table"]==table_name ]
            ### 获取基础定位信息：
            # 例子：col_list_basic = ["S_INFO_WINDCODE","WIND_CODE","ANN_DT","REPORT_PERIOD" ]
            col_list_basic= list( df_sub[ df_sub["if_needed"]==2 ]["name"].values )
            
            ### "if_needed"]==1对应财务指标，"if_needed"]==2对应基础信息如股票代码，基准年度等
            df_sub2 = df_sub[ df_sub["if_needed"]==1 ]
            col_list = list( df_sub2["name"].values )

            col_list_full =col_list_basic+col_list
            ### 期末数据文件前缀,后缀
            file_prefix = df_sub["file_prefix"].values[0]
            file_sufix  = df_sub["file_sufix"].values[0]
            ...
            ########################################################
            ### 1，导入盈利预测明细：需要获取最近的5~10个预测数据，对应多个交易日（假定180天）
            ''' notes:AShareConsensusData在每个预测日EST_DT只对当天有预测值的股票统计历史数据，也就是说每个交易日都是不全的个股数据
            但是 AShareConsensusRollingData 只对过去180天的数据做统计，没有30和90天的统计值;
            Qs:AShareConsensusData 按预测日期看，过去180天平均每只股票平均有63个预测统计的数据，中位数28个；最多540个，最少1。
            感觉还不如用 中国A股盈利预测明细[AShareEarningEst] 和 AShareConsensusData 进行汇总
            Wind一致预测个股滚动指标[AShareConsensusRollingData]：对应过去180天预测数据，如果没有说明过去180天没有预测值。
            notes:AShareConsensusData单日一般只有几十到200,400个股票，因此需要提取最近30个交易日的数据
            '''
            if table_name in ["AShareEarningEst" ] :
                count_days = 0 
                # 取最近30个交易日
                for temp_date_sub in date_list_sub[-30:] : 
                    file_name = file_prefix + str(int(temp_date_sub)) + file_sufix + ".csv"
                    # D:\db_wind\data_wds\AShareConsensusData 
                    
                    ### notes:某个交易日数据有可能不存在
                    try :
                        df_0 = pd.read_csv(path_table + file_name  )
                    except:
                        df_0 = pd.read_csv(path_table + file_name,encoding="gbk"  )
                    
                    if count_days == 0 : 
                        ### 仅保留部分columns
                        df_esti_detail = df_0 
                    else :
                        df_esti_detail = df_esti_detail.append( df_0 ,ignore_index=True )      

                    count_days = count_days +1
                ### 剔除重复项：
                df_esti_detail = df_esti_detail.drop_duplicates(subset=["OBJECT_ID"],keep="last")
                
                df_esti_detail = df_esti_detail.loc[:, col_list_full]

            ########################################################
            ### 2，导入盈利预测汇总：中国A股盈利预测汇总[AShareConsensusData]数据只需要最新值
            if table_name == "AShareConsensusData" :
                count_days = 0 
                # 取最近10个交易日
                for temp_date_sub in date_list_sub[-30:] : 
                    file_name = file_prefix + str(int(temp_date_sub)) + file_sufix + ".csv" 
                    # D:\db_wind\data_wds\AShareConsensusData 
                    ### notes:某个交易日数据有可能不存在
                    ### df_esti_stat是最近30、90、180天的统计汇总
                    try :
                        df_0 = pd.read_csv(path_table + file_name  )
                    except:
                        df_0 = pd.read_csv(path_table + file_name,encoding="gbk"  )
                    if count_days == 0 : 
                        ...
                        df_esti_stat = df_esti_stat.append( df_0,ignore_index=True )      

                    count_days = count_days +1 
                ### 剔除重复项：
                df_esti_stat = df_esti_stat.drop_duplicates(subset=["OBJECT_ID"],keep="last")
                
                df_esti_stat = df_esti_stat.loc[:, col_list_full]

        ##################################################################################
        ### save to obj_fi
        obj_fi["df_esti_detail"] = df_esti_detail
        obj_fi["df_esti_stat"] = df_esti_stat
        
        # print("Check number of stock for 30,90days ",len( df_esti_detail.index ) ,len( df_esti_stat.index )  )
        # Check number of stock for 30,90days  9994 55409
