...
'''
===============================================
 1分钟不超过5次！您每天最多访问该接口20次

derived from data_io_wind.py
date:last   | since 180601
===============================================
'''
#######################################################################
import sys,os
...
class data_ts():
    def __init__(self):
        #######################################################################
        ### 
        self.path_data ='D:\\db_wind\\tushare\\'
        ### Initialization TUSAHRE;url= https://tushare.pro/webclient/
        # 导入tushare
        import tushare as ts
        # 初始化pro接口
        self.pro = ts.pro_api('610356f7536b274912eb18c2198135266ab7e097cda7dabedb12c074')
        

    def print_info(self):
        ###
        print("class data_ts() operates data  ")
        print("log_data_ts || 建立本地数据更新日志，判断每个环节是否已经更新。")

        print("get_ts_stock_basic || 股票基础数据")
    ...
    def get_ts_stock_basic(self,obj_data):
        #######################################################################
        ### 获取股票基础数据
        #######################################################################
        ### 股票列表 
        df = pro.stock_basic(**{ "ts_code": "", "name": "", "exchange": "", "market": "", "is_hs": "", "list_status": "", "limit": "", "offset": ""}, 
    ...
    def get_ts_fund_basic(self,obj_data):
        #######################################################################
        ### 获取基金数据 | D:\db_wind\tushare\fund_nav
        path_output = self.path_data + "fund_nav\\"
        
        if obj_data["type"] == "nav" :  
            ### 判断是否存在该文件
            file_name = "ts_fund_nav_" + obj_data["temp_date"] + ".xlsx"
            if os.path.exists( path_output + file_name ) :
                df_date = pd.read_excel( path_output+ file_name  )
                ...
                # df = self.pro.fund_nav(ts_code='165509.SZ')
                ### nav_date	str	N	净值日期 （二选一）
                df_date = self.pro.fund_nav( nav_date= obj_data["temp_date"] )

                ####################################
                ### save to xlsx ||
                # print("Debug  === ", type( df_date ) , path_output+ file_name  )
                if len( df_date.index ) > 10 :
                    # try :
                    df_date.to_excel( path_output+ file_name  )
                    # except: 
                        # df_date.to_excel( path_output ,encoding="gbk" )
            ###
            obj_data["df_nav_date"] = df_date 

        if obj_data["type"] == "basic" : 
            ####################################
            ### 基础信息
            ### 场内基金
