    ...
    def quote_concat_csv(self,temp_code,temp_date,list_name,path_data):    
        ...
        if not os.path.exists( path_data+temp_file_2 ) :
            ...
            if not "DATE" in df_stock.columns :
                df_stock["DATE"] = df_stock["Unnamed: 0"]
                df_stock = df_stock.drop(["Unnamed: 0"],axis=1)
                df_stock.to_csv(df_stock )
        else :
        ...
        ### If it is Monday, then we minus 2 when calculating date difference
        # 判断是否周一，但是还是无法解决中国节假日的问题。
        if temp_dt.isoweekday() == 1 :
            dt_diff = temp_dt - dt_hist - dt.timedelta(2)
        else :
        ...
        if dt_diff> dt.timedelta(1) :
            ...
            # temperery wind to csv | D:\data_Input_Wind\temp
